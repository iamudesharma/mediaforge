...
description: Use when building or modifying Flutter UI that must adapt to multiple form factors (mobile, tablet, desktop, foldables, resizable windows). Covers LayoutBuilder, MediaQuery.sizeOf, Expanded/Flexible, ConstrainedBox, and breakpoint-driven layouts. Use when the user mentions responsive layout, adaptive layout, tablet layout, large screen, foldables, breakpoints, or making the UI look good on both phone and desktop.
metadata:
  model: deepseek/deepseek-chat
  source: https://github.com/flutter/skills/blob/main/skills/flutter-build-responsive-layout/SKILL.md
