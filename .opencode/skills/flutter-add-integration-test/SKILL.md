...
description: Use when adding Flutter integration tests, wiring up `integration_test/`, `test_driver/integration_test.dart`, enabling `flutter_driver` extension, assigning `ValueKey`s for test targeting, or running `flutter drive` on Chrome, Android, iOS, or Firebase Test Lab. Also use when converting interactive MCP exploration (launch_app, get_widget_tree, tap, enter_text) into permanent `WidgetTester`-based tests, or capturing performance timelines via `traceAction`.
metadata:
  model: deepseek/deepseek-chat
  source: https://github.com/flutter/skills/blob/main/skills/flutter-add-integration-test/SKILL.md
