...
Native engine: [`video_forge`](../video_forge/). Disk filmstrip cache: [`video_forge_cache`](../video_forge_cache/) (included by default).

## Quick start

```yaml
dependencies:
  video_forge_kit: ^0.2.0
  path_provider: ^2.1.5
```
...
Depend only on `video_forge` for FRB, or `video_forge_kit` without pulling cache-only APIs — see [VIDEO_PACKAGE_SPLIT.md](../../docs/VIDEO_PACKAGE_SPLIT.md).
