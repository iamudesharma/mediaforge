...
use image_forge::api::image::{decode_blurhash, encode_blurhash};
...
fn apply_filter_variants_and_brightness() {
    ...
    .expect("dark");
    let rb = image_forge::api::advanced::decode_to_rgba_buffer(bright, false, None).unwrap();
    let rd = image_forge::api::advanced::decode_to_rgba_buffer(dark, false, None).unwrap();
...
fn rotate_rgba_arbitrary_changes_dimensions() {
    init_app();
    use image_forge::api::advanced::rotate_rgba_arbitrary;
