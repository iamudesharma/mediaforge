...
pub fn apply_glow_rgba(buffer: &mut RgbaImageBuffer, strength: f32) {
    ...
    for chunk in glow_src.chunks_exact_mut(4) {
        let luma = (chunk[0] as f32 * 0.299 + chunk[1] as f32 * 0.587 + chunk[2] as f32 * 0.114) / 255.0;
        if luma < 0.55 {
            chunk[0] = 0;
            chunk[1] = 0;
            chunk[2] = 0;
...
pub fn apply_grain_rgba(buffer: &mut RgbaImageBuffer, strength: f32) {
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            for c in 0..3 {
                buffer.pixels[i + c] = (buffer.pixels[i + c] as f32 + noise * amp)
...
pub fn apply_halation_rgba(buffer: &mut RgbaImageBuffer, strength: f32) {
    ...
    for y in 0..h {
        for x in 0..w {
            let i = (y * w + x) * 4;
            let luma = (src[i] as f32 * 0.299 + src[i + 1] as f32 * 0.587 + src[i + 2] as f32 * 0.114) / 255.0;
            ...
            if luma > 0.6 {
                ...
                buffer.pixels[i + 1] = (src[i + 1] as f32 * (1.0 - t * 0.1)).clamp(0.0, 255.0) as u8;
                buffer.pixels[i + 2] = (src[i + 2] as f32 * (1.0 - t * 0.1)).clamp(0.0, 255.0) as u8;
            }
        }
    }
}
...
pub fn apply_dewy_highlight_rgba(buffer: &mut RgbaImageBuffer, strength: f32) {
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            if lift > 0.001 {
                for c in 0..3 {
                    buffer.pixels[i + c] = (buffer.pixels[i + c] as f32 + lift).clamp(0.0, 255.0) as u8;
                }
            }
        }
    }
