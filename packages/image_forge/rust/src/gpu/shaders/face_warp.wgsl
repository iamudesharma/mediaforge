...
fn main(@builtin(global_invocation_id) gid: vec3u) {
    ...
    let cx = params.cx * w;
    let cy = params.cy * h;
    let radius = params.radius_norm * min(w, h);
    let fx = f32(x);
    let fy = f32(y);
    let dx = fx - cx;
    let dy = fy - cy;
    let dist = length(vec2f(dx, dy));
    if (dist > radius || dist < 1.0 || abs(params.strength) <= 0.001) {
        output_rgba[idx] = input_rgba[idx];
        return;
    }
    let t = 1.0 - dist / radius;
    let push = params.strength * radius * pow(t, params.falloff);
    let len = max(dist, 1.0);
    let sx = clamp(fx - dx / len * push, 0.0, w - 1.0);
    let sy = clamp(fy - dy / len * push, 0.0, h - 1.0);
