...
use crate::api::image::{RgbaImageBuffer, Rotation};
...
pub fn rotate_rgba_arbitrary(
    ...
    let rotated = rotate_about_center(&img, radians, Interpolation::Bilinear, Rgba([0, 0, 0, 0]));
    Ok(RgbaImageBuffer {
        width: rotated.width(),
...
pub fn rotate(img: DynamicImage, rotation: Rotation) -> DynamicImage {
    match rotation {
        ...
        Rotation::FlipVertical => {
            DynamicImage::ImageRgba8(image::imageops::flip_vertical(&img.to_rgba8()))
        }
    }
}
