...
/// Normalized lip center for plump warp (CPU + GPU).
pub fn lip_center_norm(analysis: &FaceAnalysisResult) -> (f32, f32) {
    let regions = match LandmarkRegions::from_analysis(analysis) {
        Some(r) => r,
        None => return (0.5, 0.65),
    };
    lip_center_from_analysis(&analysis.landmarks, regions)
}
...
pub fn apply_beauty_rgba(
    ...
    if params.lip_plump > 0.001 {
        ...
        let center = lip_center_norm(analysis);
