...
/// One radial warp pass for GPU `face_warp.wgsl` (Sprint 22).
#[derive(Clone, Copy, Debug)]
pub struct GpuWarpSpec {
    pub cx: f32,
    pub cy: f32,
    pub radius_norm: f32,
    pub strength: f32,
...
/// Collect warp passes to mirror [apply_face_warp_rgba] without CPU readback.
pub fn collect_gpu_warp_specs(
    analysis: &FaceAnalysisResult,
    params: &BeautyParams,
) -> Vec<GpuWarpSpec> {
    let mut specs = Vec::new();
    let landmarks = &analysis.landmarks;
    if params.eye_enlarge > 0.001 && !landmarks.is_empty() {
        let (cx, cy) = centroid(landmarks);
        specs.push(GpuWarpSpec {
            cx,
            cy,
            radius_norm: 0.06,
            strength: params.eye_enlarge * MAX_EYE_ENLARGE,
            falloff: 1.15,
        });
    }
    if params.nose_slim > 0.001 && !landmarks.is_empty() {
        let (cx, cy) = centroid(landmarks);
        specs.push(GpuWarpSpec {
            cx,
            cy,
            radius_norm: 0.04,
            strength: -params.nose_slim * MAX_NOSE_SLIM,
            falloff: 0.85,
        });
    }
    let n = analysis.face_contour_count.min(landmarks.len() as u32) as usize;
    if n >= 8 {
        let (cx, cy) = centroid(&landmarks[..n]);
        if params.jaw_slim > 0.001 || params.chin_vshape > 0.001 {
            let strength = params.jaw_slim * MAX_JAW_SLIM + params.chin_vshape * MAX_CHIN_V;
            specs.push(GpuWarpSpec {
                cx,
                cy,
                radius_norm: 0.22,
        ...
        if params.face_slim > 0.001 {
            specs.push(GpuWarpSpec {
                ...
                cy,
                radius_norm: 0.35,
                strength: -params.face_slim * MAX_FACE_SLIM,
                falloff: 0.9,
            });
        }
    }
