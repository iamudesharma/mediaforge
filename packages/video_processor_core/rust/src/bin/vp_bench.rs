//! Benchmark harness for flutter_video_processor (same Rust pipeline as the Flutter plugin).
//!
//! Usage:
//!   cargo run --release -p video_processor_core --bin vp_bench -- \
...
//!   cargo run --release -p video_processor_core --bin vp_bench -- --prefer-hardware
...
use video_processor_core::pipeline::{extract_batch_thumbnails, extract_thumbnail, probe_media_info, run_compress};
use video_processor_core::types::{
