...
/// Resolves and opens `libvideo_processor_core` for the current platform.
abstract final class NativeLibraryLoader {
  static const String stem = 'video_processor_core';

  static Future<ExternalLibrary> load() async {
    // iOS: linked via CocoaPods vendored_frameworks — not a loose CodeAsset dylib.
    if (Platform.isIOS) {
      // Linked via CocoaPods vendored_frameworks (see ios/flutter_video_processor.podspec).
      for (final name in [
        'video_processor_core.framework/video_processor_core',
        _bundledLibName(),
      ]) {
        try {
  ...
  static String _platformHint() {
    ...
    if (Platform.isIOS) {
      return 'iOS: from repo root run:\n'
          '  ./scripts/run-ios.sh\n'
          '  (builds FFmpeg + video_processor_core.framework)';
    }
    if (Platform.isMacOS) {
      return 'macOS: from repo root run:\n'
          '  ./scripts/run-video-macos.sh\n'
          '  (cargo build --release; Flutter hook bundles libvideo_processor_core.dylib)';
    }
  ...
  static List<String> _candidatePaths() {
    ...
    final paths = <String>[
      ...
      '$cwd/../../../target/release/${_dylibFileName()}',
      '$cwd/../../../../target/release/${_dylibFileName()}',
      '$cwd/../../packages/video_processor_core/rust/target/release/${_dylibFileName()}',
      '$cwd/../packages/video_processor_core/rust/target/release/${_dylibFileName()}',
      '$execDir/../Frameworks/${_dylibFileName()}',
      '$execDir/../Frameworks/$stem.framework/$stem',
      '$execDir/Frameworks/${_dylibFileName()}',
      '$execDir/Frameworks/$stem.framework/$stem',
      '$cwd/../packages/video_processor_core/macos/Frameworks/$stem.framework/$stem',
      '$cwd/../../packages/video_processor_core/macos/Frameworks/$stem.framework/$stem',
      '$cwd/packages/video_processor_core/macos/Frameworks/$stem.framework/$stem',
    ];
    ...
    if (Platform.isMacOS || Platform.isIOS) {
      paths.addAll([
        ...
        '${Platform.environment['HOME']}/.flutter_video_processor/${_dylibFileName()}',
      ]);
    }
