...
mod imp {
    ...
    ///
    /// `CVMetalTextureCacheCreateTextureFromImage` takes a **Metal** pixel
    /// format, not a CoreVideo one — passing `K_CV_32BGRA` there aborts inside
    /// `MTLDebugValidateMTLPixelFormat`.
    pub const K_MTL_BGRA8_UNORM: u32 = 80;
    ...
    impl CvMetalTexture {
        ...
        pub fn clone_metal_texture(&self) -> MetalTexture {
            // `metal::Texture` is generated by `foreign_obj_type!` with
            // `fn clone = obj_clone`, i.e. its `Clone` sends `retain` and its
            // `Drop` sends `release`. Using `Texture::from_ptr` here instead
            // would adopt a `+1` that nobody owns, producing a double release
            // once both wrappers are dropped.
    ...
    pub fn create_bgra_iosurface_pixel_buffer(
        ...
        // NULL attributes: CoreVideo applies its own defaults.
        create_bgra_iosurface_pixel_buffer_with(device, width, height, false)
    }

    /// Same as [`create_bgra_iosurface_pixel_buffer`] but with the IOSurface
    /// backing and `kCVPixelBufferMetalCompatibilityKey` requested explicitly.
    ///
    /// Used as the enhancement output surface: the buffer is written by a GPU
    /// pass and then handed to Flutter's `Texture` widget, which only adopts it
    /// without a CPU copy when both attachments are present (see
    /// `canAdoptPixelBufferDirectly` in the Darwin plugin).
    pub fn create_bgra_iosurface_pixel_buffer_metal(
        device: &metal::Device,
        width: u32,
        height: u32,
    ) -> Result<IosurfacePixelBuffer, String> {
        create_bgra_iosurface_pixel_buffer_with(device, width, height, true)
    }

    fn create_bgra_iosurface_pixel_buffer_with(
        device: &metal::Device,
        width: u32,
        height: u32,
        explicit_metal_attributes: bool,
    ) -> Result<IosurfacePixelBuffer, String> {
        if width == 0 || height == 0 {
        ...
        unsafe {
            ...
            //    IOSurface-backed + Metal-compatible (both are defaults); the
            //    explicit variant spells them out for the adoption check.
            let mut pixel_buffer: CVPixelBufferRef = ptr::null_mut();
            let mut attrs: *mut std::ffi::c_void = ptr::null_mut();
            if explicit_metal_attributes {
                attrs = metal_compatibility_attributes()?;
            }
    ...
    ///
    /// The caller owns the returned `+1` CFDictionary and must `CFRelease` it.
    unsafe fn metal_compatibility_attributes() -> Result<*mut std::ffi::c_void, String> {
        unsafe {
            let empty_iosurface_props = CFDictionaryCreate(
                kCFAllocatorDefault,
                ptr::null(),
                ptr::null(),
                0,
            ...
            if empty_iosurface_props.is_null() {
                return Err("CFDictionaryCreate (empty) failed".into());
            }
            ...
            let values: [*const std::ffi::c_void; 2] =
                [empty_iosurface_props, kCFBooleanTrue as *const std::ffi::c_void];
            ...
            CFRelease(empty_iosurface_props);
            if attrs.is_null() {
                return Err("CFDictionaryCreate (attrs) failed".into());
            }
            Ok(attrs)
        }
    }

    /// Metal texture view of an existing BGRA `CVPixelBuffer`.
    ///
    /// Borrows the caller's pixel buffer — the returned [`CvMetalTexture`]
    /// holds its own retain on the `CVMetalTextureRef` (which in turn retains
    /// the pixel buffer), so the caller keeps ownership of its own `+1` and
    /// must release it separately.
    ///
    /// Returns `Err` for a null pointer, a non-BGRA pixel buffer or a buffer
    /// that is not Metal-compatible, so the caller can fall back to the plain
    /// render path instead of producing a black frame.
    ///
    /// # Safety
    ///
    /// `pixel_buffer` must be a valid `CVPixelBufferRef` that stays alive for
    /// the duration of the call.
    pub unsafe fn metal_texture_view_for_pixel_buffer(
        device: &metal::Device,
        pixel_buffer: CVPixelBufferRef,
        width: u32,
        height: u32,
    ) -> Result<CvMetalTexture, String> {
        if pixel_buffer.is_null() {
            return Err("pixel buffer is null".into());
        }
        if width == 0 || height == 0 {
            return Err(format!("pixel buffer size {width}x{height} is invalid"));
        }
        unsafe {
            let format = CVPixelBufferGetPixelFormatType(pixel_buffer);
            if format != K_CV_32BGRA {
                return Err(format!(
                    "pixel buffer format 0x{format:08X} is not BGRA (0x{K_CV_32BGRA:08X})"
                ));
            }
            let cache = metal_texture_cache_for_device(device)?;
            let mut cv_mtl_texture: CVMetalTextureRef = ptr::null_mut();
            ...
            // Unretained view → take our own +1 so the `metal::Texture` Drop
            // balances against the CV ref that also owns a reference.
            let _: *mut std::ffi::c_void = objc::msg_send![mtl_raw, retain];
            let metal_texture = metal::Texture::from_ptr(mtl_raw);
            Ok(CvMetalTexture {
                cv_texture: cv_mtl_texture,
    ...
    /// Lock a BGRA `CVPixelBuffer` and hand its bytes to `f` as
    /// `(pixels, bytes_per_row)`.
    ///
    /// Diagnostic helper: locking forces a GPU→CPU sync point, so it is never
    /// on a render path. Used to verify enhancement output in tests and to
    /// dump a surface when debugging.
    ///
    /// # Safety
    ///
    /// `pixel_buffer` must be a non-null, valid `CVPixelBufferRef` of BGRA8
    /// type that the caller keeps alive for the duration of the call.
    pub unsafe fn with_bgra_pixels<R>(
        pixel_buffer: CVPixelBufferRef,
        f: impl FnOnce(&[u8], usize) -> R,
    ) -> Result<R, String> {
        unsafe {
            with_locked_pixels(pixel_buffer, |base, len, stride| {
                f(std::slice::from_raw_parts(base, len), stride)
            })
        }
    }

    /// Mutable variant of [`with_bgra_pixels`] — same locking rules, for
    /// filling a surface (tests, synthetic input).
    ///
    /// # Safety
    ///
    /// Same contract as [`with_bgra_pixels`]; the caller must not keep the
    /// slice alive after the closure returns.
    pub unsafe fn with_bgra_pixels_mut<R>(
        pixel_buffer: CVPixelBufferRef,
        f: impl FnOnce(&mut [u8], usize) -> R,
    ) -> Result<R, String> {
        unsafe {
            with_locked_pixels(pixel_buffer, |base, len, stride| {
                f(std::slice::from_raw_parts_mut(base, len), stride)
            })
        }
    }
    ...
    /// # Safety
    ///
    /// See [`with_bgra_pixels`].
    unsafe fn with_locked_pixels<R>(
        pixel_buffer: CVPixelBufferRef,
        f: impl FnOnce(*mut u8, usize, usize) -> R,
    ) -> Result<R, String> {
        if pixel_buffer.is_null() {
            return Err("pixel buffer is null".into());
        }
        unsafe {
            ...
            let result = if base.is_null() {
                Err("CVPixelBufferGetBaseAddress returned NULL".into())
            } else {
                Ok(f(base, bytes_per_row * height, bytes_per_row))
            };
