...
### Public API
- **`bufferPoolAcquireWithToken(minCapacity) -> (Uint8List, BigInt)`** (sync FFI). Returns the buffer paired with a monotonically increasing `releaseToken`. Use the token with `bufferPoolReleaseByToken` to return the buffer to the pool. Token `0` is reserved for "no token" and is a no-op.
- **`bufferPoolReleaseByToken(token) -> void`** (sync FFI). Releases a buffer by its `releaseToken`. The current Rust pool is a single shared best-fit free list, so this is a no-op for the pool's internal accounting — but the FFI export is wired through so the Dart-side `Finalizer` can fire-and-forget.
- **`decodePreviewFrameRgbaBuf(...) -> PreviewFrameRgbaBuf`** (async FFI). Same as `decodePreviewFrameRgba` but the returned struct includes a `releaseToken: BigInt`. Wire it through `ReleaseToken.fromPreview` and let the `Finalizer` return the buffer when the parent object is GC'd.
- **`PreviewFrameRgbaBuf` struct** — `{ ptsMs, width, height, rgba, releaseToken }`. New code should prefer this over the bare `PreviewFrameRgba` (kept for back-compat).
- **`ReleaseToken` class** in `lib/src/release_token.dart`. Wraps a `Uint8List` + `releaseToken` and attaches a `Finalizer` so the underlying buffer is returned to the Rust pool when no Dart references remain. Exposes `detach()` for cases where the buffer needs to outlive the `ReleaseToken` (e.g. hand it to a worker isolate).

### Performance
- **Pool token is opaque across FFI.** The token is `AtomicU64` on the Rust side, generated by `NEXT_TOKEN.fetch_add(1) + 1` (token `0` is reserved for "no token"). The Dart `Finalizer` only sees a `BigInt`; there's no way to mis-use it as a pointer.
- **Finalizer-based release is best-effort.** Errors from `bufferPoolReleaseByToken` are swallowed (the Rust side is a no-op anyway, and the existing explicit `bufferPoolRelease(Uint8List)` path still works).

### Tests
- 3 new Rust unit tests in `pool::tests` (token round-trip, token 0 no-op, token increments).
- 3 new Dart tests in `test/types/release_token_test.dart` (construct, detach idempotent, zero-token no Finalizer).
