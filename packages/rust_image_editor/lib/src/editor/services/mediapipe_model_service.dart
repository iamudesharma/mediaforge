...
abstract final class MediaPipeModelService {
  ...
  static Future<bool> isMediaPipeReady() async {
    ...
    if (!File('$dir/$faceFileName').existsSync() ||
        !File('$dir/$segmenterFileName').existsSync()) {
      return false;
    }
    // macOS does not link MediaPipeTasksVision; native reports SDK + files.
    if (Platform.isIOS || Platform.isMacOS) {
      try {
        const channel = MethodChannel('rust_image/face');
        final ready = await channel.invokeMethod<bool>(
          'isMediaPipeReady',
          {'modelDir': dir},
        );
        return ready == true;
