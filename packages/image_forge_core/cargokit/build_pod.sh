...
case "${PLATFORM_NAME}" in
  macosx)
    PREBUILT="${CARGOKIT_MANIFEST_DIR}/../macos/Prebuilt/libimage_forge_core.a"
    ;;
  iphoneos|iphonesimulator)
    PREBUILT="${CARGOKIT_MANIFEST_DIR}/../ios/Prebuilt/libimage_forge_core.a"
    ;;
esac

if [ -n "${PREBUILT}" ] && [ -f "${PREBUILT}" ]; then
  mkdir -p "${CARGOKIT_OUTPUT_DIR}"
  cp "${PREBUILT}" "${CARGOKIT_OUTPUT_DIR}/libimage_forge_core.a"
else
