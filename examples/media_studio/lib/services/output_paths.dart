...
class OutputPaths {
  ...
  /// True when outputs live under `output/` in the repo project folder.
  ...
  static Future<OutputPaths> resolve() async {
    ...
    // Phones cannot write to the dev machine's project `examples/media_studio/output/` tree.
    if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
      final docs = await getApplicationDocumentsDirectory();
      final root = p.join(docs.path, 'media_studio_example', 'output');
  ...
  static Future<String> studioScratchDir() async {
    ...
    final dir = Directory(p.join(base.path, 'media_studio_scratch'));
